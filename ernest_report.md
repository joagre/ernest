...
**Float arithmetic.** IEEE 754 binary64, round to nearest, ties to even, restricted to the finite range. An operation whose result is not finite faults with cause `Fault("float arithmetic error")`: overflow, division of a non-zero numerator by zero, or `0.0 / 0.0`. Gradual underflow to a subnormal or a signed zero is not a fault. There is no `Infinity` and no `NaN`; `Float.compare`, `Float.round`, `Float.floor`, and `Float.ceil` are total.
...
6. A module is documented as a section 3 manual page, in CommonMark (§2.2). The module's doc block says what the module is for, then has the sections `Examples`, with one central example, and `See also`. Every declaration has a doc block, a member of an abstract type at its signature entry: one sentence saying what the type does not say, which occurrence `remove` removes, the order `toList` produces, the range `next` draws from; an `Errors` section when it faults, and none otherwise (rule 4); an `Examples` section with one example for an exported `type` or `fn`, an abstract type's examples covering its members, and none required for a `let`, whose sentence says what the value is, the example ending in `// => v` where `v` is what `Io.debug` prints for its value; `See also` when there is something to see; and, as the doc block's last line, `since v`, the toolchain version in which the declaration appeared, on the module's doc block too. The name and the type are the heading and the code block `ernc --doc` renders (§11.4).

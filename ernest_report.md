...
**Standard output and standard error.** `stdout` writes each text it receives to standard output as its UTF-8 bytes, and each `Bytes` as they are, and adds no newline. `stderr` does the same to standard error. It is a second sink, for a program whose output is read by something else, and not a level of severity.

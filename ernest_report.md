...
Three faults are not operations. The loss of a peer faults every process on it with `Fault("peer lost")` (§10). A deadlock faults the entry process with `Fault("deadlock")` (§8.6). The replacement of a process's code under it faults the process with `Fault("its code was replaced")` (§11.2).

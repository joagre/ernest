    ...
    case ern_typecheck:check(Ns, Decls, DepIfaces) of
        {ok, Typed, _, Env} ->
            ...
            ["# Ernest module ", qname(Ns), "\n\n", ModDoc,
             [doc_decl(D, Prefix, Entries, Env) || D <- Typed, documented(D)],
...
signature(#foreign_fn_decl{owner = O, name = N, params = Ps, ret = R, effect = E}, Prefix, _) ->
    code([Prefix, owned(O, N), " : ", syn(#t_fn{params = [T || #param{type = T} <- Ps], ret = R,
                                                 effect = E})]);

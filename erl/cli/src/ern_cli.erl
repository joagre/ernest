...
doc(Opts, Path, Err) ->
    case filelib:is_dir(Path) of
        ...
        false ->
            ...
            try
                io:put_chars(doc_text(beam_of(Opts, Path))),
                0
            catch
...
doc_dir(Opts, Path) ->
    ...
    Entries = [begin
                   ...
                   ok = file:write_file(Out, unicode:characters_to_binary(doc_text(Beam))),
                   ["- [", qname(Ns), "](", Rel, ")\n"]
...
doc_text(Beam) ->
    ...
    {ok, {docs_v1, _, ernest, _, ModDoc, Meta, Entries}} = ern_emitter:read_docs(Beam),
    ...
    ["# Ernest module ", qname(Ns), "\n\n", ModText,
     [entry_text(E, Prefix, Members) || E <- Entries],
     "---\n\nGenerated by ernc ", ?VERSION, " from ",
     binary_to_list(maps:get(source, Meta, <<>>)), ".\n"].
...
entry_text({{_, Name, _}, _, Signature, Doc, Meta}, Prefix, Members) ->

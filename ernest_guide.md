...
`Tab` completes the word before the cursor, by its prefix or by its word starts, `L.fM` to `List.filterMap`. It offers only what may stand there: a command after a leading `:` and what the command takes after it, a type after `:` in an annotation, a constructor in a pattern, a field inside a named constructor's parentheses. A second `Tab`, or one with nothing to add, lists what it offers under the line, until the next key. `Shift-Tab` shows the type, the first sentence, and the version of the name at the cursor, and pressed again its documentation. Inside a call it shows the callee's signature, the argument at the cursor in colour.

...
7. How the upgrade case is typed, and what an in-place upgrade may change (section 8). The limit on the message type follows from typed addresses. The limit on the state type follows only if the upgrade case carries a function from the current state type, which depends on how code change is typed in `ernest.md`. To be settled there, together with whether replacement with state handover needs support from the language or can stay a convention.

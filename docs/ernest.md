...
**`recv`.** `recv { arms }` is a match over the mailbox. The mailbox is scanned in arrival order; the first message that matches some arm is removed, the rest remain, and the arm is evaluated. If none matches, the process waits until one arrives. Patterns are typed against the mailbox type. The arms need not cover the type; a message no arm matches stays in the mailbox. A final arm `after t -> e` gives a time limit in milliseconds, evaluated on entry; after `t` without a matching message, `e` is evaluated. `after 0` does not wait for new messages. Without `after` there is no limit.

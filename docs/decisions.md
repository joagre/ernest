...
**What writing it said about the language.** Ernest has no field selection, so every function that reads two fields of a record opens by destructuring it, and reading one boolean is a function of its own. In the screen, which is six fields carried through a dozen functions, that is the most repetitive thing in the shell. It is not a workaround and nothing is wrong with the code; it was the first place where the absence cost something, and it was decided the next day under "Field selection" in Later: it stays out.

...
- **Raw string literals.** A literal in which `\` is not an escape, so that a regular expression reads `\d+` rather than `\\d+`, and a Windows path or embedded JSON likewise. A general lexical form, never a regex literal: regular expressions are a library (MVP 2.7), as in Gleam, and Elixir's `~r` is one case of a sigil mechanism Ernest does not have. The cost is a second string syntax beside `"..."`. Admitted if MVP 2.7's regex examples read badly with doubled backslashes; the design question is then the delimiter.
- **String interpolation.** `"count is ${n}"` as Elixir's `#{}`; an external reviewer asks for it, and printf-style output is what developers write. Two designs: holes of type `String` only, a lexical convenience over `<>`; or any type, which needs a per-type `toString` resolved as §4.8 resolves `+`, since Ernest has no protocols. Against it: a second way to build a string, an expression grammar inside a token, and an implicit call per hole. `Io.debug` covers the development case. Admitted when three programs build a message from three or more `<>` pieces; the design question is then the hole's type.

...
- **Field selection.** `s.upper` for a field of a constructor, as OCaml, Haskell and Gleam have it. Not settled: it is an item of MVP 2.65, a discussion after the shell is built, so that it is judged on the code the shell made. What was argued when it first came up, on principle 2: a pattern already reads a field, and a selector is a second way to do the same job, in a language whose records are the constructor's fields rather than a type of their own. Three shapes exist to copy. Haskell's selector is a function on the type and is partial, throwing at run time on a constructor without that field. OCaml refuses it for exactly Ernest's shape, an inline record inside a constructor, and Standard ML's `#f` projects a structural record, which Ernest does not have. Gleam's is the best of them and was the first framing here to be wrong: a field is read with a dot when every constructor of the type has a field of that name and type, so the accessor exists exactly when it is total and is refused otherwise. There is nothing for a reader to be surprised by, and "only where the type has one constructor", which this entry first gave as the shape Ernest would need, was too narrow. Gleam is also where a reader most likely comes from, since Ernest took `Con(..x, f = v)` for update from that family and would then have update without read, which is the asymmetry such a reader would notice; that is the strongest argument for adding it, and it is an argument about the resulting code rather than about a count. What decides it is principle 1's test, code that a reader would not predict from the requirement. The shell's screen was the first program to feel the absence, and one thing to try before the language is a record of six fields made smaller; if the reading still does not read as a reader would write it, that is the evidence the discussion wants.

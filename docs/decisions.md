...
- **String interpolation.** `"count is ${n}"` as Elixir's `#{}`; the user's mentor asks for it, and printf-style output is what developers write. Two designs: holes of type `String` only, a lexical convenience over `<>`; or any type, which needs a per-type `toString` resolved as §4.8 resolves `+`, since Ernest has no protocols. Against it: a second way to build a string, an expression grammar inside a token, and an implicit call per hole. `Io.debug` covers the development case. Admitted when three programs build a message from three or more `<>` pieces; the design question is then the hole's type.
...
The paper phase is done: web server, file sync, snake, REPL, and the web server compared with Erlang. The background queue was dropped; it existed for backpressure, which waits. The Unison and Gleam weeks were dropped. MVP 1 is done through plan 3.3, 2026-09-17, and 3.5, the paring of the report, 2026-09-18: the chain from `.ern` to BEAM holds, the nine example programs run through `ernc` and `ern`, and the standard library is Erlang under the ABI. Appendix E has its rules, 2026-09-18, and plan 3.4, type error placement, is done the same day; tag `mvp1`. MVP 2's first three items, `Float` and operators, foreign code, and bitstrings, are done 2026-09-18, and their three open points closed 2026-09-19. Next: MVP 2's fourth item, abstract-type ownership, then the rest of MVP 2, the whole report on one node; then MVP 2.5, the standard library in Ernest under `stdlib/`, with the report edits its step 1 names first; then MVP 2.6, the first four libraries under `libs/`, driven by one command-line program, with their appendix in the report.

...
The paper phase is done: web server, file sync, snake, REPL, and the web server compared with Erlang. The background queue was dropped; it existed for backpressure, which waits. The Unison and Gleam weeks were dropped. MVP 1 is done, 2026-09-18, tag `mvp1`: the chain from `.ern` to BEAM, the example programs running through `ernc` and `ern`, the standard library as Erlang under the ABI, Appendix E's rules, type error placement, and the paring of the report. MVP 2 is done, 2026-09-19, tag `mvp2`: the whole report on one node, the shell moved to MVP 2.6 to be written in Ernest. The documentation norm, MVP 2.5's step 0, was done the same day. Next: MVP 2.5 from step 1, the standard library in Ernest under `stdlib/`, report first; then MVP 2.6 and 2.7, the first-party libraries and the shell; then MVP 3, distribution with content addressing and the library fetcher.

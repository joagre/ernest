...
- **Field selection.** `s.upper` for a field of a constructor, as OCaml and Haskell have it. Declined on principle 2: a pattern already reads a field, and a selector is a second way to do the same job, in a language whose records are the constructor's fields rather than a type of their own. Principle 1 decides the shape it would have to take: on a type with several constructors a selector is partial, which Haskell allows and pays for with selectors that throw at run time, so Ernest could only offer it where the type has one constructor — and a reader who knows `Event = Key(Key) | Resized(Size)` would expect `e.rows` to work somewhere. OCaml refuses it for exactly this shape, an inline record inside a constructor; Standard ML's `#f` is a projection on a structural record, which Ernest does not have. What would change it: code that a reader would not predict from the requirement, which is principle 1's test. The shell's screen was the first program to feel the absence, and the answer there is to look at a record of six fields before looking at the language; if that record is made smaller and the reading still does not read as a reader would write it, the question comes back.

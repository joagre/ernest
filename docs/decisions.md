...
## The Shell's Three Decisions, 2026-09-19

The shell's design note, `docs/shell_design.md`, holds the shell's design; three of its choices were the user's and are recorded there under "Decisions" with the alternatives not taken. In short: multi-line input ends at a blank line, since a type whose alternatives follow on later lines parses as complete too early; each input is its own small module, so a redefinition never changes a function already defined (principle 1); and each input runs in its own process with its own inferred mailbox type, since a session has no single mailbox type a user could choose, at the price of `self()` not outliving its input and of Erlang's `flush()`.

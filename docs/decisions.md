...
Not taken: a bare integer as a size, `x:16`, which Erlang allows and the grammar does not; a string literal as a segment, since a `String` is not `Bytes` and `String.toUtf8` says what is meant; hexadecimal literals, which §2.5 leaves to the standard library.

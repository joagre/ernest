...
- **`Address.call`.** Request and reply as one function: send with a fresh reply address, wait on it, `Optional` on timeout. Every paper program wrote it by hand. A prelude function, one line per use, and the runtime may short-circuit it to a direct call for foreign processes. Left out of the report until it has been written three times more.
...
- **Backpressure.** The mailbox is unbounded; a fast producer and a slow consumer fill memory until the node dies. Alternatives: a bounded mailbox where `send` blocks (Go), a `send` that can fail (`Either`, one match per send), or a credit protocol by convention (Erlang in practice, Pony in the runtime). Leaning toward the convention, documented as a deliberate loss. To be decided by the background queue.

...
- **The rest is the host's alone:** `isTerminal`, `colours`, `version`, `startupFiles`, `program`, `sessionNames`, `sessionTexts`, `sourceRoot`, `Shell.History.file`, the screen's `write` and `setScreen`, `holdTerminal`, and `watchDeaths` with `mine`, which tell the session of every death but those of the shell's own processes.

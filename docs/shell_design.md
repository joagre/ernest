...
- **Each input is compiled as a module of its own**, against the environment so far, and loaded. Its namespace is `Input<n>`, counting from 1 in the session. A person sees it only where two types would otherwise print alike (below): a spawn site is written as the session writes names (§11.2), `input:1 faulted: division by zero` for a process an input's own expression spawned and `start:1` for one a function the session declared spawned. The expression is the body of a function named `'$input'`, which no Ernest identifier is spelled as, so it shadows no name the session declares, `main` among them. A module on the load path called `Input3` is unreachable in a session that has got that far.

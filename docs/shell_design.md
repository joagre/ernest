...
The shell is an Ernest program of three processes over a front end. Its source is a tree of its own, `shell/`, beside `stdlib/` and `examples/`, compiled by `make` into `build/shell/` where `ern shell` finds it. It is not standard library, which E.0 would not admit it to, and not a library, which `libs/` holds; it is the toolchain's own program, written in Ernest.

- **The session** is the entry process (§8.1). It holds the `Env`, the settings and the last hundred faults; the inputs typed meanwhile wait in its mailbox. It receives inputs from the reader, outcomes from the front end and fault reports from `Process.faults`, and sends the screen what to print. It runs no user code.
...
`ern shell [--config-dir dir] [--load-path dir]... [--source-root dir] [--main Qualified.name] [file.erc]` (§11.2). The file is optional, and `--source-root` says where `:load` finds a module's source, the working directory by default, as `ern build` defaults it.
...
- **An input reaches a program's processes through its services.** There is no registry (§6.5), so the prompt has a running program's modules, its output, its faults, and the addresses its top-level bindings hold. A program meant to be driven from the prompt declares a service, or returns an address from the function that starts it.
...
- **One module owns the terminal, `Terminal`.** It replaces `Keys`, which spoke for the keyboard alone: the terminal is the resource, and it sends two kinds of event. `Terminal.subscribe(wrap)` delivers `Terminal.Event`, the one list of what the terminal sent — the characters and keys, and `Resized(Size)` — or answers `Left(NotATerminal)` where standard input is not a terminal, and `Terminal.size()` answers the size now, `Some(Size(rows, columns))`, or `None` where standard output is not a terminal (E.16). §8.2's system process is the same one renamed, and its reference is `Terminal`'s.
- **The shell owns it, and the runner records the holder.** The runner starts the session's processes, so it marks the reader, or in line mode the session that reads the lines, as the terminal's holder before any input runs. `Terminal.subscribe` and `Io.readLine` from any other process end the caller with `Fault("the shell holds the terminal; run the program with ern to give it the keyboard")`. §8.2 gains no notion of a shell: a holder is recorded, and the runner is the one that can record it, because it made the process. The shell reads through `Terminal` and `Io.readLine` as any program does.
- **In line mode the holder holds standard input**, and the fault is the same. Line mode is what the session uses when output has no size or the reader's subscription is refused, input not being a terminal: `Io.readLine`, no editing.
...
- **The screen writes and nothing else does.** A program's print is an ordinary send; ordering is the screen's mailbox order. §8.2 is untouched: the runtime starts the system processes and binds their addresses, and text written with `Io.print` still reaches standard output.
...
- **The terminal's width is the only measurement.** It clips a row of the tail that would wrap, and wraps the input and what `Tab` and `Shift-Tab` show: an input clipped at the edge hid what was typed past it and pinned the cursor there, so each of its lines wraps onto the rows below, the cursor placed on the row and column it falls on; `Terminal.size` answers it and `Resized` says when it changed, which repaints the region.
...
- **The tail never takes more than half the screen.** It shrinks to what is left when the window is short or the input is long, since the line being typed must be visible; `:set output n` asks for rows and does not command them. What `Tab` and `Shift-Tab` show takes the rows under the input that the screen has left, and counts the lines it has no room for on its last row.
...
- **A paste is one input.** The shell turns on the terminal's bracketed paste; a pasted text, blank lines included, is submitted by the `Enter` after it, and arrives as E.16's `Pasted`.
- **Functions that call each other are entered in one input.**
- **Each input is compiled as a module of its own**, against the environment so far, and loaded. Its namespace is `$Input<n>`, counting from 1 in the session, which no program can name, since `$` is in no identifier (§2.3). A person sees it only where two types would otherwise print alike (below): a spawn site is written as the session writes names (§11.2), `input:1 faulted: division by zero` for a process an input's own expression spawned and `start:1` for one a function the session declared spawned. The expression is the body of a function named `'$input'`, which no Ernest identifier is spelled as, so it shadows no name the session declares, `main` among them.
...
- **A `let` binds as a block `let` does**, monomorphic and allowed to be effectful, which is what `let a = Chat.start()` and `let a = spawn(Local, f)` need. A top-level `let` would be neither: §4.6 generalizes one whose initializer has no effect, and an input's value is not a service of the session. Its pattern binds each name it holds, `let #(a, b) = pair`, the input being `{ let p = e; #(names) }`; `let _ = e` binds none, and `let x <- e` is refused, since no block follows it to end.
...
- **A later input may add a member to a type the session declared** (§11.2). The session keeps a member under its type's name and the input that declared it, and an operator on the type looks there; a type declared again starts with no members. An abstract type's constructor stays its own input's (§4.4), so a member that takes one apart is still declared with the type.
...
- **An input that does not check is shown as `ern build` shows an error** (§11.5), the input as the source and the span underlined. Nothing is run.
...
- **The shell cannot monitor what it cannot address.** There is no registry, and a process is reached only through an address someone holds, a service's binding among them (§6.5).
- **The session subscribes, as any program may.** `Process.faults` delivers every fault as it happens, with the process, the spawn site and its line, and the cause (E.21); the session decides which are news.
...
- **The program's processes, not the shell's own.** The session keeps its own processes, itself, the screen and the reader, by their `Process`, which has equality where an address has none (E.21). An input's process catches its own fault, its outcome, so no report of it comes but for a fault a signal brought, and that report is the input's answer. A binding `:load` evaluates is caught the same way. So an input's fault is reported once. A process `:reload` ends is reported as any fault is.
- **One line, written by the screen**, as all output is.
- **The last hundred faults are kept** in the session's state, and `:faults` prints them, oldest first.
...
- **Reporting the program's own quiescence is later**.
...
- **Moving:** `C-a`, `C-e` to the start and end of the line; `C-b`, `C-f` a character; `M-b`, `M-f` a word, a run of letters and digits; the arrow keys.
- **Deleting:** `Backspace` and `C-h` back, `C-d` forward; on an empty line `C-d` quits.
- **Killing:** `C-k` to the end of the line, `C-u` to the start, `C-w` back to a space, `M-Backspace` the word before, `M-d` the word after, a word being a run of letters and digits as for `M-b`; `C-y` yanks the last kill, which outlives the line it was killed from.
...
- **Colour**, from `Shell.Style`, at a terminal and never with `NO_COLOR` set: a fault, a diagnostic's first line, and every refusal of a command red, an answer plain, a printed value's type dimmed, a name bold in a listing and a brief, the parameter at the cursor cyan, and in documentation a heading and strong emphasis bold, emphasis in italics, and a code span cyan. The region measures a row without its escape sequences.
...
- **`Tab` completes a qualified name segment by segment:** `Li` to `List.`, `List.ma` to `List.map`. After a name the session binds or a module exports, or a selection from one, it completes the fields its type selects, `it.co` to `it.count`: the front end checks the text before the last `.` in a module of its own, as `Shift-Tab` checks its callee, and asks the checker which fields that type selects (`ern_typecheck:fields/2`, over §3.5's own rule), so a chain is the checker's; a name the unfinished input binds is not in scope there, as §11.2 says. A typed segment reaches only the names of as many segments, so `a` reaches `answer` and not `Address.call`; a namespace, a module or a type with members such as `Address`, completes with its dot, and is listed as `module List` or `namespace Address`; it is offered only where it holds a name that may stand there, so after `:` a namespace that holds no type is not. An input's own module is never offered, and an operator is no name and does not complete. A constructor and a field are listed with their types, as a value is. A segment's first letter matches as typed, since its case says what a name is: `a` reaches no constructor. What the prefix matches share is completed and never less than was typed; abbreviations alone complete their namespace and keep the last segment, `L.fM` to `List.fM`, since what `filterMap` and `flatMap` share, `f`, would drop the `M` that narrowed them.
- **A second `Tab` lists the candidates**, each with its type, and so does a first `Tab` that adds nothing to the line: `:` and `Tab` lists the commands at once. A lone candidate is completed as far as it goes, a module with its dot and a command or a setting that takes a value with its space, `:set dep` to `:set depth `, and is listed whenever `Tab` reaches it, a name with its type and a command or a setting with its help line. A function completes without a `(` after it: in Ernest a function is a value passed by name as often as it is called, `List.map(xs, Int.toString)`, so the `(` is a choice and not a completion; Erlang's shell adds one because there a bare name is never a value. The listing stands in the region under the input, as Erlang's shell, zsh and fish show theirs, and the next key takes it away. The `Typing` message carries the rows shown under the line, so every key says what stands there, nothing for most keys, and the line and its listing are painted in one write.
- **Candidates are matched by prefix and by abbreviation:** `S.pS` and `String.ps` complete to `String.padStart`, matching capitals and segment starts. They are listed alphabetically, by prefix before by abbreviation. With nothing typed the candidates are the starting points, the names the session declares, the modules in scope, and the prelude's names other than its constructors, `spawn` and `send` among them, which no module leads to: listing every name, the prelude's constructors of system messages first, was three hundred rows of what a person rarely types, and every other name completes from its first letters.
- **What completes, by position:**
  - in an expression, the bindings, the modules in scope, and their values and constructors;
  - after `:` in a type annotation, types;
  - inside a named constructor, in construction, update (`Player(..p, `), or pattern, its fields;
  - at the start of an input, after `:`, a command; after a command, what it takes: for `:type` an expression, as anywhere else; for `:doc` any name; for `:browse` the modules in scope, listed by their names alone; for `:load` the modules under the source root, read a directory at a time, the one the typed segments name, so a large root is never walked; for `:forget` the values and types the session declares; for `:set` its four words, and after `timing` the word `on` or `off`. Only the argument's first word completes, except an expression's. After a command that takes nothing, or `:output`'s path, nothing completes.
- **`Tab` indents where only spaces stand before the cursor on its row**, four spaces, which is the style guide's rule and what a person does on a continuation row. A tab character is never inserted: in a line it is invisible, where spaces are what they look like (§0, principle 3), and the region would have to paint it to a stop. Elsewhere, with nothing before the cursor to complete, `Tab` lists what may stand there, after `List.map(` as after a command; four spaces inside a call were an indent nowhere.
...
- **A command completes as a word of the shell's own**, `:br` to `:browse ` and its help line under the line, with the space where the command takes an argument, and a second `Tab` lists the commands it could be with their help lines. The parser is not asked: a command is not the language's.
- **`Shift-Tab` shows documentation**, under the input as a listing is. On a name, the whole name the cursor stands in: its type, its first sentence, and its `Since`; a second `Shift-Tab`, the section `:doc` prints. Inside a call, where no name before the cursor is documented: the signature with its parameters as declared, `circle(centre : Point, radius : Int) -> Shape`, the parameter at the cursor in the terminal's cyan, or between asterisks where there is no colour, since the mark carries meaning; a prelude function shows its parameters' types alone, since the prelude names none. A constructor shows its fields as a signature, `Point(x : Int, yval : Int) -> Point`, the field whose value is at the cursor marked and none where a field's name stands. The call is looked for with the input read as an expression, as a block's statement, and as declarations, and after a command in its argument, so a `let`, a `fn` body and `:type` find it. The brief on a name ends with its `Since`, its own or its module's. The parser says which call and which argument (`#diag.within`, the innermost call), `ern_types:format_call/4` prints the signature in three parts around that argument, and the shell colours the middle one; the region measures a row without its escape sequences and resets a colour it cuts short.
- **Later, by type**, once the checker can check an unfinished input:
  - at `match e {`, `Tab` inserts one clause per constructor of `e`'s type, each body `fault("todo")`, and in a partial `match` the clauses the exhaustiveness check finds missing (§5.9);
...
The list of commands is §11.2's and `Shell.Command.commands`'s, which `commands_mirror_test` holds equal; what follows is their design.
...
- **`:load Module`** — the module by its namespace, never a path. Its source is found under the shell's source root and compiled as `ern build` would compile it; a module with no source there, the standard library's or a library's, is loaded from its compiled form. A source under the source root wins over an `.erc` on the load path. Its top-level bindings are evaluated before it is in scope, each module's in a process of the front end's own whose fault is `:load`'s answer rather than a fault line (`initialize/1`, §11.2). Afterwards the module is in scope by its qualified name, like anything else on the load path. A standard library module is in scope from the start, and `:load` says so, since §4.2 makes its namespace taken; a name whose segments do not each begin with a capital letter is refused as no module's, by `:load` and `:browse` alike.
- **`:reload`** — every loaded module whose source differs from the one it was compiled from, as `:load` would take each; it takes no name, `:load Module` being that already. What is compared is the source's hash against the one the compiled module carries (§11.1), not a modification time. A call from another module reaches the new code. A process running the module's own loop does not: it keeps the version it is in until it returns, which is the case §6.10 exists for. A closure made from the previous code keeps that code while the version lives. The first reload names the processes and bindings in the previous version, by spawn site and by name, and says that a further reload of that module ends them; that reload ends them and says which, the processes with the cause of §7.4. A death `:reload` reports is not reported twice: the fault reports leave out a process it ended.
- **`:quit`** — quits, erasing the live region and leaving the cursor on a fresh line.
- **`:doc Name`** — the documentation of `Name`, the page `ern doc` writes (§11.4), rendered for the terminal, the declaration included. Every name that completes after `:doc` has some: a constructor shows its type's section, where §11.4 documents it; a module the head of its page, its title, version, and doc block, since what it declares is `:browse`'s; a name the session declares is headed as the session writes it and shows the type the shell prints, not the input's namespace; and a `let` at the prompt has its name and type. A module `:load` compiled is read from the session's copy, since it is loaded from memory and has no file. A name that is both a type and a module, `List`, shows the type's section and then the module's head, and a namespace that is neither, `Net` where only `net/http.ern` is, lists what it holds. The page is rendered for the terminal by `libs/markdown`, which the shell is built against and ships with, at the screen's width; the brief on a first `Shift-Tab` is read from the parsed page, the first line of its first code block, the first sentence of its first paragraph, and its version.
- **`:help`** — the commands, alphabetically; it says that `:doc` is what GHCi calls `:info`.
...
- **`:processes`** — the live processes by their spawn sites (§6.9), read with `Process.live` and `Process.info`, the shell's own left out; a site, never an address.
...
- **The rest is the host's alone:** `colours`, which reads `NO_COLOR`, `version`, `startupFiles`, `program`, `sessionNames`, `sessionTexts`, `sourceRoot`, `Shell.History.file`, the screen's `write` and `setScreen`, and `holdTerminal`. The shell reads what runs and what faulted through `Process`, as any program does.

## Prerequisites

Delivered before the shell. Those marked report first are written into the report before the code.

- **`Terminal`** delivers the arrows, `Enter`, `Escape`, and characters, and the window's size as `Resized` (E.16's `Terminal.Event`). A sequence that is none of those arrives as `Escape` and the characters after it, which is how `Meta` and `Shift-Tab` reach the editor.
- **`Terminal.Event` gains one value for the terminal's interrupt** (E.16), delivered only to the terminal's holder. A subscriber receives `Terminal.Event` values and nothing else (E.16), so without it the byte cannot arrive. Report first.
...
- **Completion is tested on the foreign entries' answers:** a prefix and an environment in, candidates out; `Shell.Complete`'s own `Test` values, run by `ern test`, test the matching.
...
2. **The terminal and the live region.** `Terminal` replaced `Keys` and gained `size` and `Resized`, and the `Key` type folded into one flat `Terminal.Event`, decoded by our own decoder: `io_ansi:scan` was measured and does not fit. The panes it was first built with were replaced the next day by the live region above: the transcript committed to the terminal, the shell painting only the bottom rows, and `:output <path>` for a second window.

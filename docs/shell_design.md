...
- **`Shift-Tab` shows documentation.** On a name: its type, its first sentence, and its `Since`; a second `Shift-Tab`, the section `:doc` prints. Inside a call, where no name before the cursor is documented: the signature with its parameters as declared, `circle(centre : Point, *radius : Int*) -> Shape`, the parameter at the cursor between asterisks, the emphasis of the markdown the pages are written in; a prelude function shows its parameters' types alone, since the prelude names none. The brief on a name ends with its `Since`, its own or its module's. The parser says which call and which argument (`#diag.within`, the innermost call), and `ern_types:format_call/4` prints the signature.

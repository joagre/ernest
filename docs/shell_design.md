...
- **Candidates are matched by prefix and by abbreviation:** `L.fM` and `List.fm` complete to `List.filterMap`, matching capitals and segment starts. They are listed alphabetically, by prefix before by abbreviation. With nothing typed the candidates are the starting points, the names the session declares and the modules in scope: listing every name, the prelude's constructors of system messages first, was three hundred rows of what a person rarely types, and every other name completes from its first letters.

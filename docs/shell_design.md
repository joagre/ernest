...
- **A second `Tab` lists the candidates**, each with its type, and so does a first `Tab` that adds nothing to the line: `:` and `Tab` lists the commands at once. The listing stands in the region under the input, as Erlang's shell, zsh and fish show theirs, and the next key takes it away. The `Typing` message carries the rows shown under the line, so every key says what stands there, nothing for most keys, and the line and its listing are painted in one write.
...
- **A command completes as a word of the shell's own**, `:br` to `:browse`, and a second `Tab` lists every command with its help line. The parser is not asked: a command is not the language's.

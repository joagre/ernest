...
- **A pane is a line buffer, the incomplete line last written, an offset and a height.** The renderer takes the visible slice at the offset and paints the pane's rows, each from its start so that what was there is gone; a write appends and repaints that pane only. There are no scroll regions. A pane keeps its last thousand lines, which is its scrollback: a session never shrinks, and a program that prints forever would.
- **The transcript fills its pane from the top**, as a terminal fills a screen, and the cursor is on the row the prompt is painted on; scrolled back, it waits on the last row.
- **The reader stops echoing when there are panes.** It tells the screen the line being typed, and the screen paints it after the prompt, since a repainted pane must draw that line again. It is the shape the line editor needs anyway.
- **A terminal shorter than eight rows is not split**, there being no room for two panes and a rule.
- **The upper pane is small and appears with the first program output.** Nothing is split until something prints, so a shell with no program running looks as it always did. It then takes up to a third of the screen, and `:set output n` fixes it at n rows, `:set output auto` gives the third back. `:set output 0` is no upper pane at all: the screen is cleared, and from then on the program's output and the shell's are written in arrival order, with the terminal's own scrollback back. There is no second setting for the split, since that one carries it.
- **`PageUp` and `PageDown` scroll the transcript, `Meta-PageUp` and `Meta-PageDown` the pane above it.** §8.2 delivers `Meta` as an `Escape` and the key after it, which is what tells the two apart. The split costs the terminal's own scrollback, since lines that leave a repainted viewport are kept by nothing, so the shell gives it back itself: the lower pane's buffer is the transcript and the upper pane's is the program's output, each with its own offset. Without these keys the older output would be unreachable rather than merely awkward, which is why they are not polish.
...
2. **The terminal and its panes.** `Terminal` replaced `Keys` and gained `size` and `Resized`, and §9.3's `Key` gained `PageUp` and `PageDown`, the keys the panes need, decoded by our own decoder: `io_ansi:scan` was measured and does not fit. What is left of the checkpoint is the panes themselves: the screen keeping two of lines, offset and height, output above and the shell below; `:set output n`; `PageUp` and `PageDown` over each pane.

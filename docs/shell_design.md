...
- **What completes, by position:**
  ...
  - at the start of an input, after `:`, a command; after a command, what it takes: for `:type` an expression, as anywhere else; for `:doc` any name; for `:browse` the modules in scope, listed by their names alone; for `:load` the modules under the source root, read a directory at a time, the one the typed segments name, so a large root is never walked; for `:forget` the values and types the session declares; for `:set` its four words. Only the argument's first word completes, except an expression's. After a command that takes nothing, or `:output`'s path, nothing completes.

...
- **The terminal's width is the only measurement.** It clips a row of the tail or the input that would wrap, and wraps what `Tab` and `Shift-Tab` show; `Terminal.size` answers it and `Resized` says when it changed, which repaints the region.

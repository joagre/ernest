...
- Errors as §11.5 says: `file:line:column: message`, then the source with the span underlined; `--errors short` for the first line alone. One `#diag{}` record from every stage, rendered by `ern_diag` (3.4).
...
- **Abstract-type ownership** (§4.4): the constructor-visibility check in the checker, in `check_values`, rejecting a constructor of an abstract type in any definition the signature does not name; the signature's types checked against the definitions as today. No compiler change: a type member lives in the Erlang module of the file that owns the type, as §11.2 says, and the earlier idea of one Erlang module per type is dropped, since the loader never looks for one. 2 days.
- **`Float`, and operators on user types** (§3.1, §4.8): finite IEEE 754 doubles, arithmetic that would produce a non-finite result faults, and `fn Distance.+` resolving as `Int.+` does. Today `ern_typecheck:resolve_operators` is a post pass: inference types `a + b` as `(t, t) -> t`, and after the definition is inferred the pass reads `t`, accepts `Int` (and `String`, `List`, `Bytes` for `<>`), refuses an unresolved `t` with "annotate it", and refuses `Float` for MVP 1. For `Int` and `Float` the post pass is exact, since `(t, t) -> t` is the operator's type. For a user type it is not: §4.8 requires only that the operands share a type, so `Vec.* : (Vec, Vec) -> Float` has a result the post pass would have typed as `Vec`, and `(a * b) + 1.0` would fail wrongly. So resolution moves into inference: when an operand's type becomes known, the operator is looked up in that type's namespace at once, its signature's result unified with the expression's, and inference continues; an operand still unknown when the definition ends is the same error as today. The report changes nowhere: §4.8 already says the operand type comes from the definition and the result from the resolved operator. 5 days, and the piece most likely to double, since nobody has written it.
- **`foreign fn` and `foreign type`** (§4.7, §8.4): compiled to a call of the named `M:F/A` inside a catch that turns an exception into `Fault`, and the returned value checked against the declared type before it is used, which is the "first observation" §8.4 promises. The compiler generates the check from the type: `Int` is an integer, `Float` a float, `Bool` `true` or `false`, `Char` a code point, `String` a binary of valid UTF-8, `Bytes` a binary, a tuple and a list checked element by element, a constructor by its tag and arity with its fields checked, `Map` and `Set` by their handles with keys and values checked, and `Foreign`, `Address`, `Reply`, and function values by shape only, since Ernest does not inspect them. A `foreign type` value is not checked. A mismatch is `Fault("foreign return does not match ...")` naming the declared type. Messages from a foreign process are checked the same way by the `receive` that binds them, generated per mailbox type, until MVP 4's runtime mailbox takes it over. The check is linear in the value's size and runs once, at the boundary. 4 days.

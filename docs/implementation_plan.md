...
**MVP 2 (the whole report on one node), about four and a half weeks:** ownership rule for abstract types with a module per type and the signature as export list (4 days); `Float` (finite IEEE 754 doubles per §3.1; arithmetic that would produce a non-finite result faults) with type-directed name resolution interleaved with inference (5 days, and the piece most likely to double, since nobody has written it); `foreign fn` and `foreign type` compiled to direct calls with a catch that turns exceptions into `Fault` (3 days); the standard library in Ernest, below (4 days); bitstrings (4 days: lexer tokens `<<` and `>>`, `BitExpr` and `BitPat` in the grammar, specifier list, type checking against `Bytes`, direct compilation to BEAM's bit syntax so the runtime's mature optimizer handles prefix-heavy protocol matches); `net` and `fs` as foreign processes (4 days); `Deadlock` (2 days); the reply discipline following function values, lifting 1.2's spawn-lambda restriction (2 days); the web server as test (3 days). The web server as test, and an `Ets.ern` as the first foreign library, with an `Erl` stdlib module for what every shim needs: `Erl.atom : (String) -> Foreign` and `type Erl.Result(v, r) = Ok(v) | Error(r)`. Under the ABI `Ok(v)` is `{'Ok', v}`, not `{ok, V}`, so an Erlang-side helper rewrites `{ok, V} | {error, R}` into the declared shape, as report Appendix D and guide §7.5 describe.

**The standard library in Ernest, 4 days, planned 2026-09-17, after `foreign fn`.** Appendix E is complete for the corpus: the eight MVP 1 programs compile against it, and the four MVP 2 programs lack only names outside it, the system references `Sys.keys`, `Sys.stdin`, `Sys.fs` (§8.2), the foreign library `Ets` (Appendix D), and `Random`, which the report does not have. Steps, in order:

1. Report first: §4.2's rule that a module namespace may not coincide with a prelude namespace gets the exception that the standard library's source root defines them, since `stdlib/list.ern` must be `List`; §8.2 decides `Random`, recommended as a system process with an `Address(RandomMsg)` in the shape of the clock, since it is not pure. Decisions log to match.
...
3. One criterion for shim against Ernest: a function is written in Ernest unless its value lives in Erlang. Pure Ernest: `Optional`, `Either`, `Bool`, `List`, `Char` but `toString`, and `Io`, whose `println` is `send(Sys.stdout, s <> "\n")`. Shims over `foreign fn`: `Map` and `Set`, whose values are Erlang maps; `String`, a UTF-8 binary, for its Unicode functions; `Float`; `Int.toString`, `Int.toFloat`, and the bit operations; `Foreign`. About thirty shims. Where Appendix E leaves a definition open, the choice the Erlang module's header states carries over.
4. Rewrite in that order, pure modules first, one module at a time. The Erlang modules of MVP 1 export Appendix E under the ABI and `lib/runtime/test/ernest_stdlib_tests.erl` tests them by that ABI, so the same tests run against the Ernest-compiled modules unchanged: the ABI stays pinned while the implementation moves, and a module is done when its Erlang original is deleted with the tests still green. `ernc` in directory mode on `stdlib/` builds it; `make` runs that.
5. Extend by the growth rule, now by counting: a function from the decisions log's deferred list enters Appendix E when three programs under `examples/` write it by hand. The report changes first, then the module.

...
10. **What was decided is built**, in seven sub-steps, each its own commit. Step 9 comes
   ...
   6. **The build, in the ledger's order**, one decision or tight group a commit, each with
      ...
      Built so far: group 1, D2, D6, D7, D8, B6 and B7, and group 2, C1, C3, C2, D1 and D4,
      on 2026-09-26; the host's interrupt
...
3. **An installation.** `make install` builds Ernest and installs it, and `make uninstall` removes what it installed. **Decided 2026-09-26: relocatable without a configure step.** The prefix is a make variable, `PREFIX`, `/usr/local` by default, and `DESTDIR` stages the tree for a package, as projects without a configure step do. Under the prefix: the toolchain's tree in `lib/ernest`, the compiled applications, the standard library, the shell, the libraries and `VERSION`; `bin/ern`, a relative symbolic link into `lib/ernest/bin/ern`, which the escript follows before it looks for its tree, so that a prefix moved or copied whole still runs; and the manual pages in `share/man/man3`, and `share/man/man1` for `ern`'s own should item 2 give it one. Erlang/OTP 29 on the path is required beside it and not bundled; a release that carries its own runtime is a later decision. [`build.md`](build.md) owns the installation once it is built, and this item writes its section there. The log's *An Installation Without a Configure Step*.

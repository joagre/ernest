...
2. The checker reads the standard library's compiled interfaces as it reads any dependency's, and `ern_prelude` shrinks to §9: the built-in types, the declared types, the built-in and process functions, the operators, `todo`, and `Sys.*`. Its stdlib types table goes with them: `Random.Seed` is then declared in `stdlib/random.ern`.

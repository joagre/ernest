...
10. **What was decided is built**, in seven sub-steps, each its own commit. Step 9 comes
   ...
   2. **Done 2026-09-26: the ledger.** A table in this plan, a row for each decision the gate
      ...
      | D5 | the shell's commands in §11.2 (step 9), built | §11.2 | `shell/shell/command.ern` | new mirror: the paragraph against the command list | `shell_design.md`, guide §1.2, §9.3, `shell/README.md` | A1, B5 | `The shell's help lists the commands` 1 |
   ...
   4. **The whole report read cold**, with the two sweeps, run 2026-09-26: a reader who took
      ...
      **Four decisions for the user**, taken before sub-step 5, each a feedback item no step
      ...
      - **Item 57, decided 2026-09-26: standard input and output carry bytes.** E.1 gains
        `Io.read`, what has arrived from standard input, and `Io.write`, bytes to standard
   ...
   6. **The build, in the ledger's order**, one decision or tight group a commit, each with
      ...
      `ern test` reports every fault but a test's own as `ern run` does (§11.2), a fault
      the runtime finds in a system process's work is the entry process's and reported as
      any is, and `examples/repl.ern` keeps its run number, since a wrap is made before the
      process it names exists (the log's *The Build's Fifth Group*). The shell's style, its

...
- **`Float`, and operators on user types** (§3.1, §4.8, §3.10): finite IEEE 754 doubles, arithmetic that would produce a non-finite result faults, `fn Distance.+` resolving as `Int.+` does, and `<` on a type that declares `compare` compiling to `T.compare(a, b) == Less`, since today the compiler emits Erlang's `<` and the checker refuses every type but the four with a prelude `compare`. Today `ern_typecheck:resolve_operators` is a post pass: inference types `a + b` as `(t, t) -> t`, and after the definition is inferred the pass reads `t`, accepts `Int` (and `String`, `List`, `Bytes` for `<>`), refuses an unresolved `t` with "annotate it", and refuses `Float` for MVP 1. For `Int` and `Float` the post pass is exact, since `(t, t) -> t` is the operator's type. For a user type it is not: §4.8 requires only that the operands share a type, so `Vec.* : (Vec, Vec) -> Float` has a result the post pass would have typed as `Vec`, and `(a * b) + 1.0` would fail wrongly. So resolution moves into inference: when an operand's type becomes known, the operator is looked up in that type's namespace at once, its signature's result unified with the expression's, and inference continues; an operand still unknown when the definition ends is the same error as today. The report changes nowhere: §4.8 already says the operand type comes from the definition and the result from the resolved operator. 5 days, and the piece most likely to double, since nobody has written it.

...
5. What "complete" means beyond that is decided by programs: `Bytes` has only `<>` until bitstrings show what else a protocol needs; a module or function from the decisions log's reading of the Erlang standard library (its "wait for a program" bin, 2026-09-18) enters when the rules of Appendix E.0 admit it, report first, and never as a composition of two that are there.

...
5. What "complete" means beyond that is decided by programs: `Bytes` has only `<>` until bitstrings show what else a protocol needs; a function from the decisions log's deferred list enters when the rules of Appendix E.0 admit it, report first, and never as a composition of two that are there.

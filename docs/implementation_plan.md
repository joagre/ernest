...
4. The system references of §8.2 as runtime processes bound by the launcher: `Sys.stdin`, `Sys.keys`, `Sys.fs`, `Sys.net`, each with its message type declared in the prelude, and `Ets` under `stdlib/` as Appendix D has it, with the `Erl` module every shim needs. The four MVP 2 programs then run and become integration tests.

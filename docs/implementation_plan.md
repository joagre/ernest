...
3. **An installation.** **Decided 2026-09-26:** `make install` and `make uninstall`, relocatable without a configure step, with `PREFIX` and `DESTDIR` as make variables, the toolchain's tree in `lib/ernest`, a relative `bin/ern` link, and the manual pages in `share/man`; [`install.md`](install.md) owns the design, and the log's *An Installation Without a Configure Step* argues it.

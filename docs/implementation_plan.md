...
- A runtime-managed mailbox, if the message check of §8.4 needs it: a buffer the runtime fills from the BEAM mailbox, so that only what a foreign process sends is walked and every receive clause loses its check. Since 2026-09-19 a `receive` guard is a guard expression by §5.9, so the mailbox no longer has to scan with compiled clause functions, and whether it is built at all is decided by measuring the per-receive check.

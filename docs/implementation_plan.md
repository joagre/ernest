...
3. **An installation.** `make install` builds Ernest and installs it under a prefix, `/usr/local` by default, and `make uninstall` removes it. Decided here, with the user, before it is built: **how it is relocatable without a configure step**, the prefix given as a make variable with `DESTDIR` for staging and the tree finding itself at run time; **the layout under the prefix**, the toolchain's tree in `lib/ernest`, `bin/ern`, and the manual pages in `share/man` or `man`; and what the installation requires beside it, Erlang/OTP 29 on the path.
4. **The release**, tagged once the review is done and the two above are built, with its notes (review phase 4f).

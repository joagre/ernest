...
**MVP 2 (the whole report on one node), about four weeks.** Each item is a report rule MVP 1 refuses or does not check; the README's table names the refusal it lifts.

- **Abstract-type ownership** (§4.4): the constructor-visibility check in the checker, and one Erlang module per type with the signature as export list. 4 days.
- **`Float`** (§3.1): finite IEEE 754 doubles, arithmetic that would produce a non-finite result faults, with type-directed name resolution interleaved with inference. 5 days, and the piece most likely to double, since nobody has written it.
- **`foreign fn` and `foreign type`** (§4.7): compiled to direct calls with a catch that turns an exception into `Fault`. 3 days.
- **Bitstrings** (§5.11): lexer tokens `<<` and `>>`, `BitExpr` and `BitPat` in the parser, the specifier list, type checking against `Bytes`, direct compilation to BEAM's bit syntax so the runtime's optimizer handles prefix-heavy protocol matches. 4 days.
- **`Deadlock`** (§8.6): the reaper tracks waiting processes and timers. 2 days.
- **The reply discipline through function values** (§6.6), lifting 1.2's spawn-lambda restriction. 2 days.
...
1. **Report first.** §4.2's rule that a module namespace may not coincide with a prelude namespace gets the exception that the standard library's source root defines them, since `stdlib/list.ern` must be `List`. Decisions log to match.
...
5. **What "complete" means beyond that is decided by programs.** Erlang's standard library was read module by module on 2026-09-18 (rationale in the decisions log); the result for the roadmap:
   ...
   - **Waiting for a program, each with its trigger:** a `Bytes` module (`size`, `get`, `slice`, `toList`, `fromList`) when bitstrings show what a protocol needs beyond `<>`; `Float.sqrt` and its family, and `Float.truncate` as the fourth rounding policy, when a program computes with them; integer bases, `Int.toString` and `String.toInt` with a base, which §2.5 promises, when a program reads hex; `Clock.monotonic` for durations, at the echo server of step 4; a `Time` module over the clock's milliseconds when a program formats a timestamp; `Array` and `Queue` only if a program needs indexed access or a FIFO that two lists cannot give. Each enters report first, by the rules of Appendix E.0, and never as a composition of two functions already there.
   - **Waiting for the first command-line program, in the report rather than a module:** `Sys.args : List(String)` and `Sys.env`, runtime-bound values in §8.2 and §9.7 like `Sys.stdout`; and an exit status, a rule for §8.6, which today ends the program when `main` returns.
   - **Libraries, never stdlib:** `base64`, `json`, `uri_string`, `re`, `crypto`, `zlib`, `dets`, `calendar`, `digraph`, `sofs`, and hashing (`phash2`, `crc32`, `md5`), on the foreign-library pattern of Appendix D.

...
**MVP 2.5 (a complete standard library for one node), about two weeks, planned 2026-09-17 and 18:** what makes Ernest useful to someone else for a single-node project, before distribution. Appendix E is complete for the corpus: the eight MVP 1 programs compile against it, and the four MVP 2 programs lack only names outside it, the system references `Sys.keys`, `Sys.stdin`, `Sys.fs` (§8.2), the foreign library `Ets` (Appendix D), and `Random`, Appendix E.13 since 2026-09-18. Appendix E was read against its rules twice on 2026-09-18, the second time under the admission and shape rules of E.0, which fixed the verbs, the argument order, and every contract the type does not state, so the rewrite implements the final shape once. The library is a mix: a function is written in Ernest unless its value lives in Erlang. Pure Ernest: `Optional`, `Either`, `Bool`, `List`, `Random`, `Char.fromInt`, and `Io`, whose `println` is `send(Sys.stdout, s <> "\n")`. Shims over `foreign fn`: `Map` and `Set`, whose values are Erlang maps; `String`, a UTF-8 binary, for its Unicode functions; `Char`'s Unicode predicates and `toString`; `Float`; `Int.toString`, `Int.toFloat`, and the bit operations; `Foreign`. Steps, in order:

1. Report first: §4.2's rule that a module namespace may not coincide with a prelude namespace gets the exception that the standard library's source root defines them, since `stdlib/list.ern` must be `List`; §8.2 lists `Sys.stdin` and `Sys.keys` beside `Sys.fs` and `Sys.net`. Decisions log to match.
...
No HTTP server, JSON, or database connectors are planned; those are libraries for others to write on the foreign-library pattern of Appendix D.

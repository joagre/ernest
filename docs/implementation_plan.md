...
- Base types `Int`, `Bool`, `Char`, `String`, `Bytes`, `Unit`; `Float` is MVP 2 and its arithmetic is refused meanwhile. Operators resolve on the operand type in one post-inference pass (`ern_typecheck:resolve_operators`): the `userop` operators (`+` to `Int.+`; `Money.+` on user types is MVP 2 with `Float`), `==` (a type error on types containing functions or addresses), `<`, `<=`, `>`, `>=` (to the operand type's `compare`, a type error without one), and `<-` by the type of `e`. `/` and `%` on `Int` fault on zero (`badarith` becomes a `Fault`); `Int.div` and `Int.mod` return `Optional(Int)`.
...
**MVP 2 (the whole report on one node), about four weeks:** ownership rule for abstract types with a module per type and the signature as export list (4 days); `Float` (finite IEEE 754 doubles per §3.1; arithmetic that would produce a non-finite result faults) with type-directed name resolution interleaved with inference (5 days, and the piece most likely to double, since nobody has written it); `foreign fn` and `foreign type` compiled to direct calls with a catch that turns exceptions into `Fault` (3 days); bitstrings (4 days: lexer tokens `<<` and `>>`, `BitExpr` and `BitPat` in the grammar, specifier list, type checking against `Bytes`, direct compilation to BEAM's bit syntax so the runtime's mature optimizer handles prefix-heavy protocol matches); `Deadlock` (2 days); the reply discipline following function values, lifting 1.2's spawn-lambda restriction (2 days). The system processes and the web server as their test are MVP 2.5, step 4. An `Ets.ern` as the first foreign library, with an `Erl` stdlib module for what every shim needs: `Erl.atom : (String) -> Foreign` and `type Erl.Result(v, r) = Ok(v) | Error(r)`. Under the ABI `Ok(v)` is `{'Ok', v}`, not `{ok, V}`, so an Erlang-side helper rewrites `{ok, V} | {error, R}` into the declared shape, as report Appendix D and guide §7.5 describe.

**MVP 2.5 (a complete standard library for one node), about two weeks, planned 2026-09-17 and 18:** what makes Ernest useful to someone else for a single-node project, before distribution. Appendix E is complete for the corpus: the eight MVP 1 programs compile against it, and the four MVP 2 programs lack only the processes behind the system modules of Appendix E.15 to E.18 and the foreign library `Ets` (Appendix D). Appendix E was read against its rules three times on 2026-09-18: the second reading wrote the admission and shape rules of E.0, which fixed the verbs, the argument order, and every contract the type does not state; the third removed what those rules did not admit, `Io.printTo`, `Io.printlnTo`, `String.any`, `String.all`, `String.toBool`, and named the sequence vocabulary that admits the rest of `List`. The rewrite implements that shape once. The library is a mix: a function is written in Ernest unless its value lives in Erlang. Pure Ernest: `Optional`, `Either`, `Bool`, `List`, `Path`, `Char.fromInt`, and the modules over the system processes, `Io`, `Clock`, `Keys`, `Fs`, `Tcp`, each a `send` or an `Address.call` to its `Sys.*` reference, as `Io.println` is `send(Sys.stdout, s <> "\n")`. Shims over `foreign fn`: `Map` and `Set`, whose values are Erlang maps; `String`, a UTF-8 binary, for its Unicode functions; `Char`'s Unicode predicates and `toString`; `Float`; `Int.toString`, `Int.toFloat`, and the bit operations; `Foreign`; `Random`, over `rand`, since a generator is the runtime's to get right (E.0, rule 1). Steps, in order:

1. Report first: §4.2's rule that a module namespace may not coincide with a prelude namespace gets the exception that the standard library's source root defines them, since `stdlib/list.ern` must be `List`. Decisions log to match. (§8.2 and §9.7 list all six references, §9.3 their message types, and Appendix E.15 to E.18 their modules, since 2026-09-18.)
2. The checker reads the standard library's compiled interfaces as it reads any dependency's, and `ern_prelude` shrinks to §9: the built-in types, the declared types, the built-in and process functions, the operators, `todo`, and `Sys.*`. Its stdlib types table goes with them: `Random.Seed` is then `foreign type Seed` in `stdlib/random.ern`.
3. Rewrite Appendix E module by module, pure modules first. The Erlang modules of MVP 1 export Appendix E under the ABI and `lib/runtime/test/ernest_stdlib_tests.erl` tests them by that ABI, so the same tests run against the Ernest-compiled modules unchanged: the ABI stays pinned while the implementation moves, and a module is done when its Erlang original is deleted with the tests still green. `ernc` in directory mode on `stdlib/` builds it; `make` runs that. From then on a rule, not a milestone: a shim exists only where the runtime alone can do the job, and a shim that stops meeting that test is rewritten in Ernest when noticed.
4. The system references of §8.2 as runtime processes bound by the launcher, `Sys.stdin`, `Sys.keys`, `Sys.fs`, `Sys.tcp`, each speaking its §9.3 type, and their modules `Io.readLine`, `Keys`, `Fs`, `Tcp` as Appendix E has them; the compiler's MVP 1 refusal of those names, `ern_compiler:refused/1`, goes, with the README's table row (4 days, moved here from MVP 2). `Ets` under `stdlib/` as Appendix D has it, with the `Erl` module every shim needs. The four MVP 2 programs then run and become integration tests. `Tcp` is processes first: `Sys.tcp` answers `Listen` and `Connect`, and each socket is a process speaking `SockMsg` over its `gen_tcp` port, so every socket is an address that `monitor`, `kill`, and `via` accept and that dies with the connection. An echo server under `examples/` measures it; if the two hops per read cost, `Tcp.read` and `Tcp.write` become foreign calls on the socket's port while the socket process keeps identity and lifetime, and no program changes a line.
5. What "complete" means beyond that is decided by programs. Erlang's standard library was read module by module on 2026-09-18 (rationale in the decisions log); the result for the roadmap:
   - **In, MVP 2.5:** the nineteen modules of Appendix E, `Ets` of Appendix D, and `Erl`. Nothing else from OTP.
   - **Waiting for a program, each with its trigger:** a `Bytes` module (`size`, `get`, `slice`, `toList`, `fromList`) when bitstrings show what a protocol needs beyond `<>`; `Float.sqrt` and its family when a program computes with them; a `Time` module over the clock's milliseconds when a program formats a timestamp; `Sys.args : List(String)` and `Sys.env` in §8.2 and §9.7 when the first command-line program is written; `Array` and `Queue` only if a program needs indexed access or a FIFO that two lists cannot give. Each enters report first, by the rules of Appendix E.0, and never as a composition of two functions already there.
   - **Libraries, never stdlib:** `base64`, `json`, `uri_string`, `re`, `crypto`, `zlib`, `dets`, `calendar`, `digraph`, `sofs`, on the foreign-library pattern of Appendix D.
   - **Out:** the behaviours, supervisors, `logger`, `application`, `code`, distribution, and every `erl_*` module; Ernest's own concepts replace them, and a supervisor is an idiom for the guide.

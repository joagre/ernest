...
- **No warts.** Never leave an approximation, a silent deviation from the report, or an unstated semantic choice in the code. The corpus rule, three programs, is one argument for a feature, not the gate: a principle can admit a feature the examples never wrote, and the log says which one did. It never excuses a defect. That no program has yet been misled by a known defect is not a reason to keep it: a defect with a known fix is fixed when found, or goes in the plan with a date if the fix is large. When the report is silent, either add the sentence to the report or reject the input with an error; never accept it silently. State every such choice to the user when it is made. A known gap goes in the plan with a date, not in a comment; a refusal made for MVP 1's sake says "MVP 1" in its error text, and a test checks the README's table lists it.

...
- **`foreign` is only what the host alone can do.** In every Ernest program we write, the shell and the examples and anything that is not the standard library, a `foreign fn` is admitted for the environment, the command line, the compiler's and the runtime's own structures, the sinks, and the terminal write that must bypass them, and for nothing else. Reading a file, splitting it, escaping it, trimming it is ordinary programming and is written in Ernest. Where it cannot be, stop and say so: the gap is a missing standard library function, a missing module, or a missing language feature, and it goes to the report, to Appendix E, or to the plan before any code goes around it. The standard library is the other side of this line, where E.0 rule 1 decides: there the runtime owns the representation, and a shim is how an operation is carried out.

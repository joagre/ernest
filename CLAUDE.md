...
- **Run the check that fits the change.** `make xref` after a change to text alone, `make emacs-mode` after one to the Emacs mode, and the whole of `make test` after any change to code.

...
- **No warts.** Never leave an approximation, a silent deviation from the report, or an unstated semantic choice in the code. When the report is silent, either add the sentence to the report or reject the input with an error; never accept it silently. State every such choice to the user when it is made. A known gap goes in the plan with a date, not in a comment; a refusal made for MVP 1's sake says "MVP 1" in its error text, and a test checks the README's table lists it.

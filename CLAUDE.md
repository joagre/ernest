...
A functional language for concurrent programs, designed by the user (joagre). Two concepts: pure functions (Hindley-Milner) and processes with typed mailboxes. See [`docs/ernest.md`](docs/ernest.md) for the full report.

## Normative structure

- **[`docs/ernest.md`](docs/ernest.md) is the single normative document.** Nothing else in this repo overrides it.
...
- **[`docs/ernest-decisions.md`](docs/ernest-decisions.md) is rationale only.** It explains *why* the report says what it says; it is updated only when [`docs/ernest.md`](docs/ernest.md) is updated. Never treat it as normative and never edit it in isolation.
...
- **The report can be updated if we find anomalies** while implementing the example programs (counter, ping-pong). When that happens, update [`docs/ernest.md`](docs/ernest.md) first, then reflect the reason in [`docs/ernest-decisions.md`](docs/ernest-decisions.md), then adjust code.
...
- **Counter** (with `Upgrade` code replacement) — see the sketch in [`docs/ernest.md`](docs/ernest.md) §6.
- **Ping-pong** — see [`docs/ernest.md`](docs/ernest.md) Appendix B.

If either test surfaces an anomaly in the report, propose an update to [`docs/ernest.md`](docs/ernest.md) (and matching [`docs/ernest-decisions.md`](docs/ernest-decisions.md)) before working around it in the parser.

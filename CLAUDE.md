...
- File and directory names use underscores everywhere (report §11.1 forbids hyphens in module paths). An Erlang module implementing an Ernest namespace, `'Ernest.Io'`, lives in `ernest_io.erl`; since `erlc` would refuse the mismatch, the Makefile compiles such files from forms with `lib/compile_ns.escript`, and only the beam is named after the atom.

...
- **Nothing routes around a defect or a gap, in code or in a document.** A toolchain bug, a missing standard library function, a shell command that cannot show something: the Ernest code, the example, the guide, the report's examples, or the test that meets it does not work around it, and does not hide it by choosing another example. The defect is fixed when found, or planned with a date when it is too large, and what needed it says that it waits. This holds for the standard library, the shell, `examples/`, the guide, and the tests alike.
